...
function getTradingSuggestion(f) {
    ...
    if (zone.includes('高位')) {
        if (trend === 0) {
            if (hasSellSignal || strongSell)
                return { action: '清仓', color: 'bg-red-600', reason: '高位下降趋势，风险较大', prediction: '未来1-2日预计继续回落，建议卖出' };
            if (strongBuy)
                return { action: '持有', color: 'bg-yellow-500', reason: '高位但有买入信号，暂持观察', prediction: '未来1-2日信号待验证，建议持有' };
            return { action: '卖出', color: 'bg-red-500', reason: '高位区域，注意风险', prediction: '未来1-2日预计偏弱，建议卖出' };
        }
        if (trend === 1) {
            if (strongBuy)
                return { action: '持有', color: 'bg-yellow-500', reason: '高位横盘+买入信号，暂持观察', prediction: '未来1-2日有望突破，建议买入' };
            if (hasSellSignal)
                return { action: '卖出', color: 'bg-red-500', reason: '高位横盘+卖出信号', prediction: '未来1-2日预计回落' };
            return { action: '减仓', color: 'bg-orange-500', reason: '高位横盘，控制仓位', prediction: '未来1-2日预计震荡调整，建议减仓' };
        }
        if (strongBuy)
            return { action: '轻仓买入', color: 'bg-green-500', reason: '高位上升+买入信号，强势延续', prediction: '未来1-2日有望上攻，建议买入' };
        return { action: '持有', color: 'bg-yellow-500', reason: '高位但仍有上升动能', prediction: '未来1-2日继续持有' };
    }
    if (zone.includes('中高位')) {
        if (trend === 0) {
            if (strongBuy)
                return { action: '持有', color: 'bg-yellow-500', reason: '中高位下降但有买入信号，暂持', prediction: '未来1-2日信号验证中，建议持有' };
            return { action: '减仓', color: 'bg-orange-500', reason: '中高位+下降趋势', prediction: '未来1-2日预计偏弱，建议减仓' };
        }
        if (trend >= 2) {
            if (strongBuy)
                return { action: '轻仓买入', color: 'bg-green-500', reason: '中高位上升+买入信号，趋势偏强', prediction: '未来1-2日有望突破，建议买入' };
            return { action: '持有', color: 'bg-yellow-500', reason: '中高位偏强，暂持', prediction: '未来1-2日建议继续持有看突破' };
        }
        if (strongBuy)
            return { action: '持有', color: 'bg-yellow-500', reason: '中高位横盘+买入信号，关注突破', prediction: '未来1-2日有望启动，建议买入' };
    ...
    if (zone.includes('中位') && !zone.includes('低') && !zone.includes('高')) {
        ...
        if (trend === 0) {
            if (strongBuy)
                return { action: '持有', color: 'bg-yellow-500', reason: '中位区下降但有买入信号，暂持', prediction: '未来1-2日信号验证中，建议持有' };
            return { action: '减仓', color: 'bg-orange-500', reason: '中位区+下降趋势', prediction: '未来1-2日预计偏弱' };
        }
        if (strongBuy)
            return { action: '持有', color: 'bg-yellow-500', reason: '中位区横盘+买入信号，关注', prediction: '未来1-2日有望启动，建议买入' };
    ...
    if (zone.includes('中低位')) {
        if (trend >= 2 && hasBuySignal)
            return { action: '轻仓买入', color: 'bg-green-500', reason: '中低位+趋势转好', prediction: '未来1-2日有反弹预期，建议买入' };
        if (trend === 0)
            return { action: '持有', color: 'bg-yellow-500', reason: '中低位下降，等待企稳', prediction: '未来1-2日可能继续探底' };
    ...
    if (trend === 0) {
        if (hasBuySignal)
            return { action: '轻仓买入', color: 'bg-green-500', reason: '低位+下降末端，有买入信号', prediction: '未来1-2日止跌反弹，建议买入' };
