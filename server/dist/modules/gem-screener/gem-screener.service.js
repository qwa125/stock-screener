...
const technical_analysis_1 = require("./technical-analysis");
...
let GemScreenerService = GemScreenerService_1 = class GemScreenerService {
    ...
    get pgSql() {
        ...
        try {
            this._pgSql = (0, postgres_1.default)(url, { max: 2, idle_timeout: 10, connect_timeout: 10 });
    ...
    async saveKlineCacheToDisk(code, data, timestamp) {
        if (!data || !data.length)
            return;
        try {
            let cache = {};
            try {
                const raw = await node_fs_1.promises.readFile(this._klineCacheFile, 'utf-8');
            ...
            await node_fs_1.promises.writeFile(this._klineCacheFile, JSON.stringify(cache), 'utf-8');
        }
        catch (e) {
    ...
    constructor(dataFetcher, stockService) {
        ...
        this._klineMemoryCache = new Map();
    ...
    loadCacheFromDisk() {
        try {
            const raw = (0, node_fs_1.readFileSync)(this.CACHE_FILE, 'utf-8');
            const parsed = JSON.parse(raw);
        ...
        try {
            if ((0, node_fs_1.existsSync)(this.BUNDLED_GEM_CACHE)) {
                const raw = (0, node_fs_1.readFileSync)(this.BUNDLED_GEM_CACHE, 'utf-8');
                const parsed = JSON.parse(raw);
                if (parsed && parsed.data && Array.isArray(parsed.data)) {
                    const limitedData = parsed.data;
                    this.cache = { ...parsed, data: limitedData };
    ...
    loadMainBoardCacheFromDisk() {
        try {
            const raw = (0, node_fs_1.readFileSync)(this.MAIN_BOARD_CACHE, 'utf-8');
            const parsed = JSON.parse(raw);
            if (parsed && parsed.data && Array.isArray(parsed.data)) {
                const limitedData = parsed.data;
        ...
        try {
            if ((0, node_fs_1.existsSync)(this.BUNDLED_MAIN_BOARD_CACHE)) {
                const raw = (0, node_fs_1.readFileSync)(this.BUNDLED_MAIN_BOARD_CACHE, 'utf-8');
                const parsed = JSON.parse(raw);
                if (parsed && parsed.data && Array.isArray(parsed.data)) {
                    const limitedData = parsed.data;
                    this.mainBoardCache = { ...parsed, data: limitedData };
    ...
    loadSectorCacheFromDisk() {
        try {
            const raw = (0, node_fs_1.readFileSync)(this.SECTOR_CACHE, 'utf-8');
            const parsed = JSON.parse(raw);
            if (parsed && parsed.data && Array.isArray(parsed.data)) {
                const limitedData = parsed.data;
        ...
        try {
            if ((0, node_fs_1.existsSync)(this.BUNDLED_SECTOR_CACHE)) {
                const raw = (0, node_fs_1.readFileSync)(this.BUNDLED_SECTOR_CACHE, 'utf-8');
                const parsed = JSON.parse(raw);
                if (parsed && parsed.data && Array.isArray(parsed.data)) {
                    const limitedData = parsed.data;
                    this.sectorCache = { ...parsed, data: limitedData };
    ...
    async saveCacheToDisk() {
        try {
            await node_fs_1.promises.writeFile(this.CACHE_FILE, JSON.stringify(this.cache), 'utf-8');
        }
        catch (err) {
    ...
    async saveMainBoardCacheToDisk() {
        try {
            await node_fs_1.promises.writeFile(this.MAIN_BOARD_CACHE, JSON.stringify(this.mainBoardCache), 'utf-8');
        }
        catch (err) {
    ...
    loadSellStateCache() {
        try {
            if ((0, node_fs_1.existsSync)(this.SELL_STATE_FILE)) {
                const raw = (0, node_fs_1.readFileSync)(this.SELL_STATE_FILE, 'utf-8');
    ...
    async saveSellStateCache() {
        try {
            ...
            await node_fs_1.promises.writeFile(this.SELL_STATE_FILE, JSON.stringify(arr), 'utf-8');
        }
        catch (err) {
    ...
    loadSnapshotFromDisk() {
        try {
            const raw = (0, node_fs_1.readFileSync)(this.SNAPSHOT_FILE, 'utf-8');
            const parsed = JSON.parse(raw);
    ...
    saveSnapshotToDisk() {
        try {
            (0, node_fs_1.writeFileSync)(this.SNAPSHOT_FILE, JSON.stringify(this.upgradedSnapshot), 'utf-8');
        }
        catch (e) {
        ...
        if (this.mainBoardCache?.data?.length) {
            for (let i = 0; i < this.mainBoardCache.data.length; i++) {
                ...
                if (upgraded) {
                    ...
                    if (upgraded.intradayHigh !== undefined)
    ...
    async fetchKlineDirect(code) {
        ...
        try {
            ...
            this.logger.warn(`⚠️ fetchKlineDirect(${code}) 失败: ${e.message}`);
            return null;
        }
    }
    async restoreKlineMemoryCache() {
        try {
            ...
            this.logger.warn(`⚠️ 恢复 K 线内存缓存失败: ${e.message}`);
        }
    }
    async runFullScan() {
        if (this._serverScanning) {
            this.logger.log('⏳ [服务端] 强制扫描已在运行，跳过');
            return;
        }
        ...
        try {
            ...
            let candidates = [];
            try {
                const j = JSON.parse(emText);
                candidates = (j?.data?.diff || []).filter((s) => s.f2 > 0 && s.f3 !== undefined).sort((a, b) => Math.abs(b.f3 || 0) - Math.abs(a.f3 || 0));
            }
            catch { }
            if (!candidates.length) {
                this.logger.warn('⚠️ [服务端] EastMoney 无数据');
                return;
            }
            ...
            this.logger.error(`❌ [服务端] 强制扫描异常: ${e.message}`);
        }
        finally {
            this._serverScanning = false;
        }
    ...
    async runLightScan() {
        if (this._serverScanning) {
            this.logger.log('⏳ [服务端] 轻量扫描已在运行，跳过');
            return;
        }
        this._serverScanning = true;
        try {
            ...
            try {
                const j = JSON.parse(emText);
                candidates = (j?.data?.diff || []).filter((s) => s.f2 > 0 && s.f3 !== undefined);
            }
            catch { }
            if (!candidates.length) {
                this.logger.warn('⚠️ [服务端轻量] EastMoney 无数据');
                return;
            }
            ...
            for (let i = 0; i < candidates.length; i += 3) {
                ...
                const batchResults = await Promise.all(batch.map(async (s) => {
                    ...
                    try {
                        ...
                        if (hasResult && cachedKline && (now - cachedKline.timestamp) < CACHE_TTL)
                            return null;
                        let kline = cachedKline?.data;
                        if (!kline || kline.length < 10 || (now - (cachedKline?.timestamp || 0)) >= CACHE_TTL) {
                            kline = await this.fetchKlineDirect(code);
                        }
                        if (!kline || kline.length < 10)
                            return null;
            ...
            this.logger.error(`❌ [服务端轻量] 扫描异常: ${e.message}`);
        }
        finally {
            this._serverScanning = false;
        }
    ...
    async onApplicationBootstrap() {
        ...
        try {
            const raw = await node_fs_1.promises.readFile(this.MAIN_BOARD_CACHE, 'utf-8');
            const parsed = JSON.parse(raw);
        ...
        if (pgGem && pgGem.data && pgGem.data.length > 0) {
            ...
            try {
                await node_fs_1.promises.writeFile(this.CACHE_FILE, JSON.stringify(pgGem), 'utf-8');
            }
            catch { }
        }
        ...
        if (pgMain && pgMain.data && pgMain.data.length > 0) {
            ...
            try {
                await node_fs_1.promises.writeFile(this.MAIN_BOARD_CACHE, JSON.stringify(pgMain), 'utf-8');
            }
            catch { }
        }
    ...
    async scanWithFrontendSectorData(stocks) {
        ...
        try {
            await node_fs_1.promises.writeFile(this.SECTOR_CACHE, JSON.stringify(this.sectorCache));
        }
        catch { }
    ...
    async generateSeedCache() {
        ...
        try {
            await node_fs_1.promises.mkdir(assetDir, { recursive: true });
            ...
            if (this.cache && this.cache.data?.length > 0) {
                ...
                await node_fs_1.promises.writeFile(gemPath, JSON.stringify(this.cache, null, 2));
            ...
            for (const [cacheFile, tmpFile] of [
                ...
                try {
                    const content = await node_fs_1.promises.readFile(tmpPath, 'utf-8');
                    const parsed = JSON.parse(content);
                    parsed.timestamp = Date.now();
                    await node_fs_1.promises.writeFile((0, node_path_1.join)(assetDir, cacheFile), JSON.stringify(parsed, null, 2));
    ...
    buildResult(s, kline, result, suggestion, signalCombination) {
        ...
        const chipResult = this.calcChipAnalysis(kline.map(k => k.close), kline.map(k => k.high), kline.map(k => k.low), kline.map(k => k.volume || 0), s.currentPrice);
        return {
            capitalRank: 0,
