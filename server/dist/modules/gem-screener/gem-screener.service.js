...
let GemScreenerService = GemScreenerService_1 = class GemScreenerService {
    ...
    async quickAnalyze(code, name, keepAll, rawKline, frontendMainForce) {
        ...
        const forecast1_2Day = (() => {
            ...
            if (ts >= 75)
                return { direction: '看涨', confidence: '较高', detail: '趋势向上+MACD向好+量价健康,未来1-2日偏多' };
            if (ts >= 55)
                return { direction: '震荡偏强', confidence: '中等', detail: '技术面偏多,短期有望延续升势' };
            if (ts >= 40)
                return { direction: '震荡', confidence: '低', detail: '各技术指标方向不一,短期方向不明朗' };
            if (ts >= 25)
                return { direction: '震荡偏弱', confidence: '中等', detail: '技术面偏弱,注意回调风险' };
