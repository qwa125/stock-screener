    ...
    async runServerSideForceScan() {
        ...
        try {
            ...
            try {
                ...
                candidates = (emJson?.data?.diff || []).filter((s) => s.f2 > 0 && s.f3 !== undefined).sort((a, b) => Math.abs(b.f3 || 0) - Math.abs(a.f3 || 0));
            }
            catch { }
            ...
            if (results.length > 0) {
                const sorted = gem_screener_service_1.GemScreenerService.sortStocks(results);
