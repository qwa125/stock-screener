...
exports.calcBaiXing = calcBaiXing;
const types_1 = require("./types");
function calcBaiXing(engine) {
    ...
    const 修正强度 = new Array(engine.length).fill(0);
    ...
    for (let i = 0; i < engine.length; i++) {
        ...
        const cond2 = 乖离率[i] > 25 && ((H[i] > refC1[i] * 1.05 && C[i] < H[i] * 0.98) ||
            ...
            (乖离率[i] > 25 && (C[i] - refC1[i]) / refC1[i] * 100 > 9 && V[i] > MA5V[i] * 2)) && (DIFF[i] < DEA[i] || C[i] < MA5[i]);
        强制覆盖[i] = cond1 || cond2;
    }
    ...
    for (let i = 0; i < engine.length; i++) {
        ...
        if (洗盘趋势强度[i] > 7)
            洗盘趋势状态[i] = types_1.TrendState.UP_STRONG;
        else if (洗盘趋势强度[i] > 4)
            洗盘趋势状态[i] = types_1.TrendState.UP_MILD;
        else if (洗盘趋势强度[i] > 0)
            洗盘趋势状态[i] = types_1.TrendState.SIDEWAYS;
        else
            洗盘趋势状态[i] = types_1.TrendState.DOWN;
    }
    ...
    const 阴跌破位 = [];
    for (let i = 0; i < engine.length; i++) {
        const countCond = engine.COUNT(覆盖中.map((v, idx) => v && C[idx] < MA5[idx] && C[idx] > MA5[idx] * 0.98 && C[idx] < (idx > 0 ? refC1[idx] : C[idx])), 3)[i];
        阴跌破位[i] = countCond >= 3 && C[i] < MA10C[i] &&
            engine.COUNT(覆盖中.map((v, idx) => v && C[idx] < MA5[idx] && C[idx] > MA5[idx] * 0.98 && C[idx] < (idx > 0 ? refC1[idx] : C[idx])), 3).map((v, idx) => v >= 3 && C[idx] < MA10C[idx])[i];
        if (阴跌破位[i]) {
            const firstCond = engine.COUNT(覆盖中.map((v, idx) => v && C[idx] < MA5[idx] && C[idx] > MA5[idx] * 0.98 && C[idx] < (idx > 0 ? refC1[idx] : C[idx])), 3);
            阴跌破位[i] = 阴跌破位[i] && engine.COUNT(firstCond.map((v, idx) => v >= 3 && C[idx] < MA10C[idx]), 本次白布开始[i] + 1)[i] === 1;
        }
    }
    const 白消涨幅 = [];
    for (let i = 0; i < engine.length; i++) {
    ...
    for (let i = 0; i < engine.length; i++) {
        ...
        const count回踩 = engine.COUNT(覆盖中.map((v, idx) => (cond回踩1 || cond回踩2 || cond回踩3)), 2)[i];
        强势回踩买点[i] = 白消状态[i] && 洗盘趋势状态[i] >= types_1.TrendState.UP_MILD &&
