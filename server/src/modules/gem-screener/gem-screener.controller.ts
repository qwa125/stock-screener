...
export class GemScreenerController {
  ...
  // 存储前端累加推送的实时报价（前端EatMoney分钟K线CORS失败时的备选方案）
  private readonly intradayBuffer: Map<string, { prices: any[]; lastPush: number }> = new Map();

  @Post('intraday-push')
  @SkipAccessLimit()
  async intradayPush(@Body() body: { code: string; price: number; volume?: number; time?: string }) {
    if (!body.code || !body.price) return { code: 400, msg: '缺少参数' };
    const now = Date.now();
    ...
    if (buf.prices.length >= 50 || (buf.prices.length >= 20 && (now - buf.lastPush) > 300000)) {
      try {
        // 将prices转换为doIntradayAnalysis需要的格式
        ...
        this.logger.warn(`[intraday-push] ${body.code} 分析失败: ${(e as Error).message}`);
      }
    }
  ...
  /** 从实时报价快照构建模拟分钟K线 */
  private buildMinuteKlineFromPrices(prices: any[]): any[] {
    if (!prices || prices.length < 2) return [];
    // 按分钟分组
    const minuteMap = new Map<string, { opens: number; high: number; low: number; close: number; volume: number; count: number }>();
    for (const p of prices) {
      const minuteKey = p.time.slice(0, 5); // HH:MM
